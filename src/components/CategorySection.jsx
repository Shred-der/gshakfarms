            ...
            <div className="md:hidden flex gap-4 overflow-x-auto no-scrollbar pb-2">
                {categories.map((cat, idx) => (
                    <button
                        ...
                        className={`flex flex-col items-center gap-2 min-w-[80px] transition-all ${activeCategory === cat.name ? 'scale-110' : 'opacity-80'}`}
                    >
                        <div className={`w-16 h-16 rounded-full ${cat.color} flex items-center justify-center transition-transform active:scale-90 shadow-sm ${activeCategory === cat.name ? 'ring-2 ring-primary ring-offset-2' : ''}`}>
                            <span className="material-symbols-outlined text-3xl">{cat.icon}</span>
                        </div>
                        <span className={`text-[11px] font-bold text-center leading-tight ${activeCategory === cat.name ? 'text-primary' : 'text-slate-600'}`}>{cat.name}</span>
                    </button>
                ))}
            </div>
